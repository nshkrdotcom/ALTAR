defmodule Altar.ADM.FunctionDeclarationTest do
  ...
  describe "new/1" do
    test "creates a valid declaration with all keys" do
      ...
      assert {:ok, %FunctionDeclaration{name: "sum_numbers", description: "Adds numbers", parameters: %{type: "object"}}} =
    ...
    test "creates a valid declaration with omitted parameters (defaults to %{})" do
      ...
      assert {:ok, %FunctionDeclaration{name: "echo", description: "Echoes input", parameters: %{} = params}} =
               FunctionDeclaration.new(attrs)
      assert params == %{}
    end

    test "fails when name is missing" do
      assert {:error, "missing required name"} = FunctionDeclaration.new(%{description: "Adds numbers"})
    end
    ...
    test "fails when name contains invalid characters" do
      ...
      for bad <- ["sum numbers", "sum!", "white space", "has.dot"] do
        assert {:error, "invalid name: must match ~r/^[a-zA-Z0-9_-]{1,64}$/ (alphanumeric, underscore, dash; max 64)"} =
